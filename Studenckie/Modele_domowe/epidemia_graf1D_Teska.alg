{Lukasz Teska 11146
Jedno-wymiarowy graf w ktorym mamy polaczenia pomiedzy sasiednimi oraz odleglymi o jeden odstep krawedziami}
{ }
Program Epidemic;
Type
node=record
...
Const
ERatio=0.5; {Ile po³aczen istnieje realnie – gdy niewaone}
Weighted=true; {Czy po³aczenia waone?}
Intensity=60; {Last*n - Ile moliwosci interakcji kadego "dnia"}
Duration=7; {Czas trwania infekcji}
Immunity=31; {Czas istnienia odpornosci}
Radius=10; {Rozmiar wez³a w wizualizacji}
Width=700; {Bok ekranu}
Ring=true; {Czy wez³y u³oone w kó³ko?}
...
{tu jest procedura implementujaca zadany graf}
Var
i:integer;
Begin
...
Repeat
j:=1+random(Last);
until i<>j; {Zak³adamy e sa co najmniej 2}
...
If w>0 then
Begin
If (TheNodes[i].state=0)and (TheNodes[j].state>0)and
(TheNodes[j].state<Duration)and(random<=w) then
TheNodes[i].state:=1;
end;
...
If v=0 then
Begin
Brush(1,0,0,155);Pen(1,0,0,155);
end
else
If v<Duration then
Begin
v:=round(255-(v-1)/(Duration-1)*155);
Brush(1,v,0,0); Pen(1,v,0,0);
end
else
Begin
v:=round(255-(v-Duration)/
(Duration+Immunity-Duration)*155);
Brush(1,0,v,0); Pen(1,0,v,0);
end;
end;
...
end; {KONIEC PROCEDURY WIZUALIZACJI GRAFU}
Var step:integer;
...
Repeat
Delay(500);
Pen(1,0,0,0); Brush(1,255,255,255); MoveTo(1,1);
CalculateStat(infected,immuned);
WriteLn(step,' ',infected,' ',immuned,' ');
ViewGraph;
{Nowy stan}
step:=step+1; TimeSlice;
until infected=0;
end.