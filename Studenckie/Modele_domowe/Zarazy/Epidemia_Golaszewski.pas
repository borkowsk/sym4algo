Program Epidemic; 
{marcin Go³aszewski Œroda 15:30
Architektura (a) A one-dimensional regular graph with each node
connected to its four adjacent nodes. Zmienione procedurê WeightedEdges} 
...
Begin 
  ...
  i:=last; 
  TheEdges[i,j]:=1;
  {Dla stworzenia ostatniego po³¹czenia} 
  i:=2; 
  j:=1; 
  Repeat 
  {po³¹czenie wêz³ów z s¹siadem (poza ostatnim)} 
    TheEdges[i,j]:=1; 
    j:=j+1; 
    i:=i+1; 
    TheEdges[i,j]:=1; 
  until i=last;
  j:=1; 
  {ostatnie po³¹czenie z co 2 s¹siadem} 
  i:=last-1; 
  TheEdges[i,j]:=1; 
  j:=2; 
  {przedostatnie po³¹czenie z co 2 s¹siadem} 
  i:=last; 
  TheEdges[i,j]:=1; 
  i:=3; 
  j:=1; 
  Repeat 
  {po³¹czenie wêz³a z co drugim s¹siadem (poza ostatnim)} 
    TheEdges[i,j]:=1; 
    j:=j+1; 
    i:=i+1; 
    TheEdges[i,j]:=1; 
