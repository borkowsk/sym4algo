...
{PROCEDURY ZMIANY STANU W TRYBIE "MONTE CARLO"}
{-------------------------------------------------------} 
...
begin 
  ...
  {Niejawnie bias-owane wyszukiwanie nowego pogl¹du}      
  max:=0; 
  nowy:=-1; 
  for i:=0 to HowManyAttid-1 do 
...
{ POWY¯EJ S¥ PROCEDURY ZMIANY STANU W TRYBIE "MONTE CARLO"}
{---------------------------------------------------------} 
...
function MeanStress(var Tab:World):real;
var 
   i,j,licznik:integer; 
   suma:real;
begin
 ...
 for i:=1 to Last do 
    for j:=1 to Last do 
      if Tab[i,j].alife then 
      begin
      suma:=suma+Tab[i,j].stres;
      licznik:=licznik+1;
      end;
   MeanStress:=suma/licznik;    
end;
...
begin 
  ...
  repeat {Powtarzaj a¿ do klikniêcia}
    MoveTo(0,Last*side+10); 
    Pen(1,0,0,0); 
    Brush(1,255,255,255); 
    CalculateStres(TheWorld); 
    Writeln(Step,' ',MeanStress(TheWorld):5:3,' '); 
