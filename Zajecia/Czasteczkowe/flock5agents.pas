Program Flock5; 
{Jedno stado albo dwa - trzeba wykomentowaæ/odkomentowaæ w programie g³ównym (na dole)}
Const 
   init_r=150; 
   Last=1600; 
   Width=500; 
...
begin 
  {PÊTLA SYMULACJI JEDNEGO STADA} 
  Step:=0; 
  While not IsEvent do 
    Begin 
      {czyszczenie} 
      DrawAll(1,Last,TheWorld,128,128,128); 
      {przesuwanie} 
      VelocityChange(1,Last,TheWorld); 
      VelocityMove(1,Last,TheWorld); 
      {rysowanie na nowych pozycjach} 
      DrawAll(1,Last,TheWorld,15,15,15); 
      {Opis na ekranie} 
      Step:=Step+1; 
      MoveTo(0,0); 
...
begin 
  {PÊTLA SYMULACJI DWUCH STAD} 
  Step:=0; 
  While not IsEvent do 
    Begin 
      {czyszczenie} 
      DrawAll(1,Last div 2,TheWorld,250,255,50); 
      DrawAll(Last div 2 + 1,Last,TheWorld,255,200,230); 
      {przesuwanie} 
      VelocityChange(1,Last div 2,TheWorld); 
      VelocityChange(Last div 2 + 1,Last,TheWorld); 
      VelocityMove(1,Last,TheWorld); 
      {rysowanie na nowych pozycjach} 
      DrawAll(1,Last div 2,TheWorld,25,20,5); 
      DrawAll(Last div 2 + 1,Last,TheWorld,205,0,25); 
      {Opis na ekranie} 
      Step:=Step+1; 
      MoveTo(0,0); 
      Write(Step,' ',MeanDistance(1,Last,TheWorld,Last div 10) :3:2); 
